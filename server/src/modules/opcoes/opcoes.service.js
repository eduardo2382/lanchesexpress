const repository = require('./opcoes.repository.js')
const { ValidationError, ConflictError, NotFoundError, UnprocessableEntityError } = require('../../error/AppError.js')

exports.existsOpcaoId= async (nome) => {
    return (await repository.findById(nome)).length > 0
}

exports.existsOpcaoArrayId = async (arr) => {
    return await repository.findOpcoesArryId(arr)
}

exports.existsOpcaoNome = async (nome) => {
...
exports.updateOpcao = async (id, body) => {
    let camposList = ['nome', 'tipo', 'status']

    if(id == undefined) throw new ValidationError('Id da opcao faltando!', {campo: 'id', motivo: 'obrigatorio'})

    let opcao = (await repository.findById(id))[0]
...
exports.updateStatusOpcao = async (id, body) => {
